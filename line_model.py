...
from _utils import check_model, uinterp1d
...
class LineModel(object):
    ...
    def __init__(self,
        ...
        for key in self._lim_params:
            ...
            self._current_params[key] = self._lim_params[key]
        ...
        if 'hubble' in self.model_par:
            print("Setting model_par['hubble'] to match cosmo_model")
            self.model_par['hubble'] = ((self.h.cosmo.H0.to(u.km/(u.Mpc*u.s))).value/100)
    ...
    def Pm(self):
        ...
        if self.nonlinear_Pm:
            P = interp1d(self.h.k,self.h.nonlinear_power)(kh)*self.Mpch**3
        else:
    ...
    def Pk_interp(self):
        '''
        Interpolator for the LIM power spectrum
        '''
        return uinterp1d(self.k,self.Pk,bounds_error=False,fill_value=0.)
    ...
    def P1D_clust(self):
        '''
        One dimensional clustering power spectrum
        '''
    ...
    def A_com(self):
        '''
        Comoving area covered by survey
        '''
        Omega_rad = (self.Omega_field.to(u.rad**2)).value
        #return self.h.cosmo.angular_diameter_distance(self.z)**2*Omega_rad
        return self.r0**2*Omega_rad

    @cached_property
    def P1D_shot(self):
        '''
        One dimensional shot noise power spectrum
        '''
        return self.Pshot/self.A_com
    ...
    def P1D(self):
        ...
        1D power spectrum
        '''
        return self.P1D_clust+self.P1D_shot
